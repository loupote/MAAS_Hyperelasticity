...
def gmshInput_mesh(inp, d=2):
            ...
            if lines[l].startswith("$Nodes"):
                ...
                for n in range(nNodes):
                    ...
                    if d == 1:
                        ...
                        mesh.addNode(lbl, float(words[1]), float(words[2]), float(words[3]))
    ...
    #
        ...
        while (l < len(lines)):
            if lines[l].startswith("$Nodes"):
                ...
                for n in range(nEntityBlocks):
                    ...
                    if param != 0:
                        raise ValueError('parametric nodes are not handled for now') # != instead of <> in Python 3
        ...
        raise ValueError('Format of the meshfile is incorrect. Verify the header $MeshFormat')
    return mesh
...
def gmshOutput_mesh(out,mesh):
    ...
    for n in range(mesh.nElements()):
        out.write("%d %d %d %d %d" % (n+1,mesh.getElement(n).getType(),2,1,1))
...
def gmshOutput_element(out,label,V,it,t):
    ...
    for n in range(np.shape(V)[0]):
        ...
        if (sz1 == 3):
            ...
            if (sz0 == 4):
                out.write(" %13.6e %13.6e %13.6e %13.6e %13.6e" % (V[n][0],V[n][1],0.0,V[n][2],V[n][3]))
                for i in range(4):
                    out.write(" %13.6e" % 0.0)
            else:
                for i in range(sz1):
